# Contributing to MongrelDB Gleam

Thanks for taking the time to help the MongrelDB Gleam client. This document
...
The MongrelDB Gleam client uses a standard **fork -> branch -> pull request**
workflow on GitHub.

1. **Fork** [`visorcraft/MongrelDB-Gleam`](https://github.com/visorcraft/MongrelDB-Gleam)
   to your GitHub account.
2. **Clone** your fork and add the upstream remote:

   ```sh
   git clone git@github.com:<you>/MongrelDB-Gleam.git
   cd MongrelDB-Gleam
   git remote add upstream https://github.com/visorcraft/MongrelDB-Gleam.git
   ```
...
5. **Open a pull request** against `master` on `visorcraft/MongrelDB-Gleam`.
...
```sh
gleam format --check src tests examples
gleam build
gleam test
```
...
```sh
MONGRELDB_URL=http://127.0.0.1:8453 gleam test
```
...
- **Version.** Gleam 1.4.0 or newer, targeting the BEAM (Erlang).
- **Dependencies.** Only `gleam_stdlib` and `gleam/http`. New dependencies
  must be MIT or Apache-2.0 licensed and justified.
- **Errors.** Use the single typed error `MongrelError` you match with a
  `case`. No throwing from library code — propagate errors as `Result`
  values.
- **Naming.** Idiomatic Gleam: `snake_case` functions and variables. Avoid
  clashing with Gleam keywords (the query builder uses `where_`, `limit_`,
  `projection` for that reason).
- **FFI.** Erlang FFI lives in `src/*.erl` alongside the Gleam modules. Keep
  it minimal and document the record/argument shapes it touches.
...
- The MongrelDB Gleam client version (from `gleam.toml` / git tag).
- Your Gleam and Erlang/OTP versions and OS.
...
The MongrelDB Gleam client is dual-licensed under MIT OR Apache-2.0. By
