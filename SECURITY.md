...
This document describes the security properties of the MongrelDB Gleam client
and how to report vulnerabilities.

## Overview

The MongrelDB Gleam client is a pure-Gleam library that talks to
`mongreldb-server` over HTTP using Erlang's built-in `httpc` (via
`gleam/http`). The client itself holds no encryption keys and stores no data
...
- **CRLF validation:** the client rejects any request whose headers contain a
  raw CR or LF, which prevents HTTP request smuggling via caller-supplied
  table names, SQL, or values.
- **Response limit:** the client caps response bodies at 256 MB
  (`max_response_bytes`); larger bodies are aborted as `ResponseTooLarge`
...
- Server and network errors are mapped to the typed error set
  (`Auth`, `NotFound`, `Conflict`, `Query`, `Http`, `Json`) you match with a
  `case`, not leaked as panics. Library code never throws — errors are
  propagated as `Result` values.

## Dependency security

The MongrelDB Gleam client depends only on `gleam_stdlib` and `gleam_http`.
...
- the MongrelDB Gleam client version, Gleam/Erlang version, and OS,
