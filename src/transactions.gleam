...
pub fn main() {
  ...
  let _ = mongreldb.drop_table(db, table)
  ...
  io.println(
    "Staged " <> int.to_string(mongreldb.txn_count(txn)) <> " operations",
  )

  let assert Ok(#(_, results)) = mongreldb.commit(txn, "")
  io.println(
    "Committed atomically: "
    <> int.to_string(list_length(results))
    <> " operations applied",
  )
  ...
  io.println(
    "After first idempotent commit: " <> int.to_string(after_first) <> " rows",
  )

  let retry2 = mongreldb.begin(db)
  let assert Ok(retry2) =
    mongreldb.txn_put(
      retry2,
      table,
      [
  ...
  io.println(
    "After duplicate idempotent commit (same key): "
    <> int.to_string(after_dup)
    <> " rows (no double-apply)",
  )
